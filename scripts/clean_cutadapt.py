    ...
    with open(snakemake.input[pair], "r") as logfile:
        ...
        while line:
            ...
            if adapter_matched:
                ...
                adapter_results[adapter_matched[0]]["Sequence"] = (
                    line_list[0].split(":")[1].strip()
                )
                if adapter_matched[0] in adapter_results:
                    adapter_results[adapter_matched[0]]["Times"] += int(
                        line_list[3].split(":")[1].split(" ")[1].strip()
                    )
                else:
                    adapter_results[adapter_matched[0]]["Times"] += int(
                        line_list[3].split(":")[1].split(" ")[1].strip()
...
with open(snakemake.output[0], "w") as outfile:
    ...
    for adapter in adapter_results_R1:
        outfile.write(
            "{},{},{},{}\n".format(
                adapter,
                adapter_results_R1[adapter]["Sequence"],
                adapter_results_R1[adapter]["Pair"],
                adapter_results_R1[adapter]["Times"],
            )
        )
    for adapter in adapter_results_R2:
        outfile.write(
            "{},{},{},{}\n".format(
                adapter,
                adapter_results_R2[adapter]["Sequence"],
                adapter_results_R2[adapter]["Pair"],
                adapter_results_R2[adapter]["Times"],
            )
        )
