...
data$presence[ids] = 'tagged'
tagged = sum(data$num_barcodes[ids])
p = ggplot(data, aes(x=num_failed_bases, y=num_barcodes, fill = presence))
p = p + geom_bar(stat = 'identity')
p = p + ggtitle(paste('Number of UMI barcode bases under', snakemake@params$min_quality,'quality in\n', snakemake@wildcards$sample,'\nTagged', format(tagged, big.mark = '\''),'Reads'))
