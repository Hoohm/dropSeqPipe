...
It uses both classical alignement (STAR) and pseudo alignement (Kallisto). Is is working for both single cell and bulk data. Although the main goal is not bulk data sequencing, don't expect good support and improvement on this end.

The short term goal is to make it usable with a maximum of existing single cell protocols.
