...
plotCumulativePlot = function(file_path, title, fraction, x_scale, path, sample_name){
  ...
  id = length(subset((y)[seq(as.integer(x_scale/4))],(y)[seq(as.integer(x_scale/4))] > fraction))
  knee_plot = ggplot(data, aes(x=x, y=y)) + geom_point(size = 0.1) +xlim(0,x_scale) + geom_vline(xintercept=as.integer(x_scale/4), linetype="dashed", color = "red") + geom_hline(yintercept=y[id], linetype="dashed", color = "red")+ ggtitle(paste0(title, '\nTotal reads: ', prettyNum(total_reads))) + theme(plot.title = element_text(size=10)) + labs(x='STAMPS', y='Cumulative fraction of reads')
...
for(i in 1:length(samples)){
  ...
  expected_cells = config_file_data$Samples[[i]]$expected_cells
  temp = plotCumulativePlot(file_path = paste0(path,"/logs/",samples[i],"_hist_out_cell.txt"), title = paste0(samples[i],"\nMinCellFraction = ", fraction), fraction = fraction, x_scale = expected_cells * 4, path = path, sample_name = samples[i])
