...
for (i in 1:length(samples)){
  temp_matrix = read.table(paste0(path,"summary/",samples[i],'_',species,'_expression_matrix.txt'), header=T, stringsAsFactors = F)
...
write.table(read_counts, file=paste0(path,"summary/",species,"_gene_name_matrix.tsv"), sep='\t')